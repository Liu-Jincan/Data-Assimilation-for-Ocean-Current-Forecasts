...
parm_WW3_input='input_era5WIND_cfosatDA' ## ～tag，新建文件需要修改～，input2?
parm_WW3_work='work_era5WIND_cfosatDA'        ## ～tag，新建文件需要修改～           ## 测试时需要更换名字，
...
gridgen_objectGrid='east-USA_P25_3_1' ## ～tag，新建文件需要修改～
gridgen_objectGrid_nml="gridgen.${gridgen_objectGrid}.nml"
gridgen_m=${programGo}".m"
gridgen_baseGrid='east-USA_P25_4_2' ## ～tag，新建文件需要修改～
...
FNAME = 'east-USA_P25_3_1'               %与namelist文件夹下的nml对应的，
FNAMEB = 'east-USA_P25_4_2'
BOUND_SELECT = 1
/
...
! -------------------------------------------------------------------- !
&GRID_NML
  GRID%NAME              =  'east-USA_P25_3_1'
  GRID%NML               =  'namelists_east-USA_P25_3_1.nml'
  GRID%TYPE              =  'RECT'
  GRID%COORD             =  'SPHE'
  GRID%CLOS              =  'NONE'
  GRID%ZLIM              =  -0.10
...
&RECT_NML
  ...
  RECT%NY                =  121
!
  RECT%SX                =   0.125000000000
  RECT%SY                =   0.125000000000
  RECT%X0                =  -77.0000
  RECT%Y0                =   33.0000
/
...
&DEPTH_NML
  DEPTH%SF             =  0.00
  DEPTH%FILENAME       = 'east-USA_P25_3_1.bot'
/

&MASK_NML
  MASK%FILENAME         = 'east-USA_P25_3_1.mask'
/

&OBST_NML
  OBST%SF              =  0.01
  OBST%FILENAME        = 'east-USA_P25_3_1.obst'
/
EOF
    ######################################################
...
declare -i ndbc
ndbc=1                                ## ～tag，新建文件需要修改～  ，一般设置为1,后面的程序会用到这里的.m程序，
...
if ((ndbc == 1)); then
    ...
    parm_ndbc_match=1                     ## 优先级3 ～tag，新建文件需要修改～      用于浮标与背景场的比较
...
if(match==1)  
    ...
    for i=1:length(fileNames)
        ...
        ndbc_start_datetime=datetime('2018-08-31 00:00:00','InputFormat','yyyy-MM-dd HH:mm:ss');
        ndbc_end_datetime=datetime('2018-09-20 00:00:00','InputFormat','yyyy-MM-dd HH:mm:ss');
...
if(match_spinup==1)
    ...
    % path_save = '/1t/Data-Assimilation-for-Ocean-Current-Forecasts/ndbc/work_eastUSA_2/';
    ndbc_spinup_datetime=datetime('2018-09-03 00:00:00','InputFormat','yyyy-MM-dd HH:mm:ss'); %一般更改2018-09-03 00:00:00即可
    HS_matchnameINtable='ww3_2018_nc_ndbc_nc_match_WVHT'; %一般更改ww3_2018_nc即可
    path_Ndbc_nc_match_Hs_Fig_spinup = strcat(path_save,'ww3_2018_nc_Ndbc_nc_match_Hs_Fig_spinup/'); %一般更改ww3_2018_nc即可
    mkdir(path_Ndbc_nc_match_Hs_Fig_spinup);
    analyse_HS_spinup(ndbc_spinup_datetime,HS_matchnameINtable,path_save,path_Ndbc_nc_match_Hs_Fig_spinup);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
%%
...
if (( DA_cycle_WithWW3_ENOI == 1 )); then 
    ...
    parm_DA_Code_ww3InputNc='nc_Background_WW3_P125_15days'                  ## ～tag，新建文件需要修改～，所需同化背景场nc所在的文件夹名称，不能为nc；用于A的生成，
    parm_DA_Code_daOuputNc='nc_WithWW3_ENOI_WW3_P125_15days_9_12_18'        ## ～tag，新建文件需要修改～，同化后nc所在的文件夹名称和文件前缀，
