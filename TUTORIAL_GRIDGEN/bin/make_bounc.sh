...
# $2 : spec directory : path where SPECTRA folder is stored
...
do
  ...
  do
    ...
    find $path_spec/SPECTRA_* -name "$spec" >> $path_d/spec.list.$mod
  done
  # sort and remove duplicated lines
  sort -u $path_d/spec.list.$mod -o $path_d/spec.list.$mod
