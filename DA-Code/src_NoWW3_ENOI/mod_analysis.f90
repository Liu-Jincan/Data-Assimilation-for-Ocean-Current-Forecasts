module mod_analysis
   use mod_params, only: N, NN, programs, generateAmatriax !, NLVLS, sub_y, sub_x, crt_bias, rgamma
   use mod_date
...
contains
   subroutine ENOI_analysis(blank, iiii, jjjj, nc_fileName, tag,Xb)
      ...
      str = trim(blank)//'3. 生成ENOI中的A，（AA’）为背景误差斜方差B，ENOI中不需要明显表示B，'&
         &//'A的生成只需要进行一次，good～，'
      if ((iiii .eq. 1) .AND. (jjjj .eq. 1)) write (*, *) str
      if (((iiii .eq. 1) .AND. (jjjj .eq. 1)) .or. generateAmatriax==1) then
         blank2 = '----'//trim(blank)//'3.'
         write (*,*) "全局变量NN的初始值：",NN
         call A_matrix(blank2,iiii,jjjj)     ! A matrix: run only 1 time before DA cycles
         write (*,*) "全局变量NN运行完后的值：",NN
         generateAmatriax = 0
      end if
