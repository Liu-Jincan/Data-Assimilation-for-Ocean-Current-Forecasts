...
contains
    subroutine sort_obs_ndbc(M2, time)
        implicit none
        integer, intent(in)  :: time(6)
        integer, intent(out) :: M2                          ! observations

        character :: filename*14
        integer   :: argos                                  ! total number of ndbc profiles, 先用argo名称进行,
        integer   :: loc(2)                                 ! loc(2)=(lon or i) and (lat or j)
        ! integer   :: lvl(2)                               ! # of stacks of bins of T & S

        integer, allocatable :: locs(:, :)                  ! horizontal locations of ndbc profiles
        integer, allocatable :: Mt(:)                       ! numbers in each argo profile
        logical   :: exist

        integer, allocatable :: index1D(:), index3D(:, :)   ! indices of obs in 1D/3D model grids
        integer, allocatable :: Tindex1Ds(:)                ! indices of obs in 1D model grids
        integer, allocatable :: Tindex3Ds(:, :)             ! indices of obs in 3D model grids
        ! integer, allocatable :: lvl_t(:), lvl_s(:)        ! # of bins of T and S
        real, allocatable    :: tmp(:)                      ! from 1 Argo
        real, allocatable    :: tmps(:)                     ! from all Argo
